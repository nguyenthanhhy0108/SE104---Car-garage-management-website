    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        if (data.Fail) {
          ...
          if (data.notMatch)
            popupDialog("Erfror", "User and mail do not match.");
        } else {
          //If sent successfully
          popupDialog(
            "Success",
            "We had sent you a verification code, please enter it for reseting password"
          );
          document.getElementById("verification_email_form").style.display =
            "none";
  ...
  $("#verification_code_form").submit(function (event) {
    ...
    $.ajax({
      ...
      success: function (data) {
        if (data.Fail) {
          if (data.expiredCode)
            popupDialog("Error", "Your code was expired, please try again!");
          if (data.wrongCode)
            popupDialog("Error", "Your code was wrong, please try again!");
        } else {
    ...
    $.ajax({
      ...
      success: function (data) {
        if (data.overlapped)
          popupDialog(
            "Error",
            "New password must not overlap with current password!"
          );
        else {
