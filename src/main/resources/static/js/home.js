...
async function deleteForm1(vehicleLicensePlate) {
  try {
    ...
    if (mainData.success == false) {
      popupDialog("Error", "You must delete all receipts first !!!");
    }
    return mainData;
  } catch (error) {
    alert(error);
...
async function fetchData() {
  try {
    ...
    function buttonEvent() {
      ...
      $(".details-button[data-vehicle-license-number]")
        .off("click")
        .on("click", function () {
          detailsData.call(this);
        });

      $(".delete-button[data-vehicle-license-number]")
        .off("click")
        .on("click", deleteData);

      $(".confirm-button[data-vehicle-license-number]")
        .off("click")
        .on("click", function () {
    ...
    function editDataForm1() {
      ...
      var inputElement = $('<input type="text" class="form-control">').val(
        currentData
      );
    ...
    async function deleteData() {
      ...
      if (vehicleID) {
        Swal.fire({
          ...
          } else Swal.fire("Cancel", "Cancel Deletion", "info");
        });
      } else {
      ...
      data.dates.forEach(function (dateData, dateIndex) {
        dateData.orderDetails.forEach(function (detail, detailIndex) {
          ...
          $('[id^="data-order-"]')
            .off("click")
            .on("click", function () {
              if ($(this).hasClass("sub-input")) return;
    ...
    async function detailsData() {
      ...
      var thisReceipts = allReceipts.filter(
        (receipt) => receipt["licenseNumber"] === vehicleID
      );
    ...
    function editDataDetails(data) {
      ...
      if (
        dataID.includes("num") ||
        dataID.includes("date") ||
        dataID.includes("price") ||
        dataID.includes("charge") ||
        dataID.includes("total")
      )
        return;
      ...
      if (dataID.includes("quantity")) {
        inputElement = $('<input type="number" class="form-control">').val(
          currentData
        );
        $(this).empty().append(inputElement);
              ...
              Swal.fire({
                ...
                if (result.isConfirmed) {
                  ...
                  var price = parseFloat(
                    $("#data-order-price-" + orderID).text()
                  );
                  var charge = parseFloat(
                    $("#data-order-charge-" + orderID).text()
                  );
                  ...
                  var returnData = createJSONformat(
                    orderID,
                    equipName,
                    price,
                    serviceName,
        ...
        dataList.forEach(function (option) {
          if (dataID.includes("equip"))
            ...
            selectElement.append(
              "<option>" + option["serviceName"] + "</option>"
            );
        });
          ...
          if (newData !== currentData) {
            Swal.fire({
              ...
              if (result.isConfirmed) {
                ...
                quantity = parseFloat(
                  $("#data-order-quantity-" + orderID).text()
                );
                var newTotal = calculateTotalPrice(quantity, price, charge);
                var equipName = $("#data-order-equip-" + orderID).text();
                var serviceName = $("#data-order-service-" + orderID).text();
